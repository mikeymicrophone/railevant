...
describe Railevance do
  describe 'caching ids' do
    ...
    before do
      ...
      @textmate = Concept.create(:content => 'TextMate')
      @bundle = Railevance.create(:rail_id => @textmate.id, :tie_r_id => @rails_ruby.id)
    end
    
    it 'should call cache_in_components when created' do
      railevance = Railevance.new(:rail_id => @rails.id, :tie_r_id => @bundle.id)
      flexmock railevance
    ...
    it 'should call cache_connections on the right object with the right id hash' do
      railevance = Railevance.new :rail_id => @rails.id, :tie_r_id => @bundle.id
      flexmock @rails
      @rails.should_receive(:cache_connections).with({:tie_r => @bundle.id}).once
      railevance.save
    end
    
    it 'should call extant_connections when cache_in_components is called' do
      railevance = flexmock(Railevance.new(:rail_id => @rails.id, :tie_r_id => @bundle.id))
      railevance.should_receive(:extant_connections).and_return({:rail => @rails.id, :tie_r => @bundle.id}).at_least.once
    ...
    it 'should cache ids in connected railevances' do
      @textmate.cached_railevance_ids.should == [@rails_ruby.id]
    end
    ...
    it 'should have the correct keys in extant connections' do
      ...
      @bundle.extant_connections.keys.include?(:rail).should == true
      @bundle.extant_connections.keys.include?(:tie_r).should == true
    end
  end
end